...
function matchResumen(m){
  ...
  const col=t=>{ const arr=Object.entries(per).filter(([p,v])=>v.t===t&&(v.g||v.a||v.sh||v.sv||v.q||v.oc))
    ...
    return `<div class="rescol ${t===1?'t1':'t2'}"><div class="resh">${name}</div>`+
      (arr.length?arr.map((x,i)=>`<div class="resrow" data-p="${x.p}"><span class="resn">${i+1}. ${nm(x.p)}</span><span class="resv">${x.v.g} G · ${x.v.a} A · <b>${x.tot}</b>${xtra(x.v)}</span></div>`).join(''):'<div class="resrow muted">Sin acciones registradas</div>')+
